export default function Results({ results, theme }) {
    ...
    if (output.error) {
      return (
        <div style={{ padding: 12, background: "#2a1010", borderRadius: 6 }}>
          ...
          {output.suggestion && (
            <p style={{ color: theme.muted, margin: "8px 0 0 0", fontSize: 13 }}>{output.suggestion}</p>
          )}
          {output.samplePath && (
            <p style={{ color: theme.muted, margin: "4px 0 0 0", fontSize: 12, fontFamily: "monospace" }}>
              Try: {output.samplePath}
            </p>
          )}
        </div>
      );
                ...
                {Object.entries(output.statistics).map(([col, stats]) => (
                  <tr key={col} style={{ borderBottom: `1px solid ${theme.border}` }}>
                    <td style={{ padding: "8px 12px" }}>
                      <code style={{ color: theme.accent, fontSize: 12 }}>{col}</code>
                    </td>
                    <td style={{ padding: "8px 12px" }}>
    ...
    return (
      <div style={{ fontSize: 14, lineHeight: 2 }}>
        <p style={{ margin: 0, color: theme.text }}>
          <span style={{ color: theme.muted }}>Title: </span>{output.title}
        </p>
        <p style={{ margin: 0, color: theme.text }}>
          <span style={{ color: theme.muted }}>Date: </span>{output.date}
        </p>
        <p style={{ margin: 0, color: theme.text }}>
          <span style={{ color: theme.muted }}>Time: </span>{output.time}
        </p>
  ...
  function renderEmailOutput(output) {
    ...
    return (
      <div style={{ fontSize: 14, lineHeight: 2 }}>
        <p style={{ margin: 0, color: theme.text }}>
          <span style={{ color: theme.muted }}>To: </span>{output.to}
        </p>
        <p style={{ margin: 0, color: theme.text }}>
          <span style={{ color: theme.muted }}>Subject: </span>{output.subject}
        </p>
        <p style={{ margin: 0, color: theme.text }}>
          <span style={{ color: theme.muted }}>Sent: </span>{output.timestamp}
        </p>
        <p style={{ margin: 0, color: theme.muted, fontSize: 12 }}>{output.status}</p>
        {output.preview && (
  ...
  function renderOutput(result) {
    ...
    if (error) {
      return <p style={{ color: theme.error, margin: 0, fontSize: 14 }}>{error}</p>;
    }

    if (tool === "csv") {
      return renderCSVOutput(output);
    }

    if (tool === "calendar" || tool === "schedule") {
      return renderCalendarOutput(output);
    }

    if (tool === "email") {
      return renderEmailOutput(output);
    }
        ...
        <div>
          {output.map((item, j) => (
            <div key={j} style={{ display: "flex", gap: 12, padding: "10px 0", alignItems: "flex-start", borderBottom: j < output.length - 1 ? `1px solid ${theme.border}` : "none" }}>
              {item.rank && (
                <span style={{ fontSize: 12, fontWeight: 700, minWidth: 28, paddingTop: 2, fontFamily: "monospace", flexShrink: 0, color: theme.muted }}>
                  #{item.rank}
                </span>
              )}
              <div style={{ flex: 1 }}>
                {item.title && (
                  <a href={item.url} target="_blank" rel="noreferrer"
                    style={{ color: theme.accent, textDecoration: "none", fontSize: 14, lineHeight: 1.4, display: "block", marginBottom: 4 }}>
                    {item.title}
                  </a>
                )}
                {item.text && !item.title && (
                  <p style={{ color: theme.text, fontSize: 14, lineHeight: 1.5, margin: "0 0 6px 0" }}>
                    {item.text}
                  </p>
                )}
  ...
  return (
    <div style={{ background: theme.card, border: `1px solid ${theme.border}`, borderRadius: 8, padding: 20, marginTop: 24, marginBottom: 60 }}>
      <h2 style={{ fontSize: 16, fontWeight: 700, marginBottom: 12, marginTop: 0, fontFamily: "system-ui, sans-serif", color: theme.text }}>
        Results
      </h2>
      ...
      {results.map((result, i) => (
        <div key={i} style={{ background: theme.bg, border: `1px solid ${theme.border}`, borderRadius: 6, padding: 16, marginBottom: 10 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 12 }}>
            ...
            {result.error && (
              <span style={{ background: "#2a1010", color: theme.error, borderRadius: 4, padding: "3px 8px", fontSize: 11, fontWeight: 700 }}>
                FAILED
              </span>
            )}
          </div>
